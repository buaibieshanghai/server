...
std::list<boost::json::object> Database::return_yewu_1(boost::json::object &obj)
{

std::cout<<"cs"<<std::endl;
    std::list<boost::json::object> list_json;
std::cout<<"hh"<<std::endl;
const char* ch =str(boost::format("insert into user_id_password values(%1%, \"%2%\",%3%,100)") % obj.at("user_zhanghao").as_string().c_str() % obj.at("user_password").as_string().c_str() % obj.at("user_image").as_string().c_str()).c_str();
std::cout<<"hh"<<std::endl;
    bool result1 = setSQL(ch);
