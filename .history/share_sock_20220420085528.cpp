...
void Share_sock::do_write()
{
    ...
    std::cout << "送" << std::endl;
            ...
            // 移除头部的json消息
            {
                ...
                if (!char_list.empty())
                std::cout << char_list.front()->data << std::endl;
...
int i=0;
 ...
 bool b= char_list.empty();
    for (auto it = list_json.begin(); it != list_json.end(); it++)
    {
        ...
        char_list.emplace_back(new Data_json(new char[index+4],index+4));
// !!!bug
