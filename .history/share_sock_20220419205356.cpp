...
void Share_sock::do_write()
{
    ...
    std::cout << "待发送" << msg.data << std::endl;
    boost::asio::async_write(

        socket_, boost::asio::buffer(data, msg.len),
        [this, self](boost::system::error_code ec, std::size_t len)
        {
            // read事件触发回调函数
...
void Share_sock::action(size_t len)
{
    ...
    for (auto it = list_json.begin(); it != list_json.end(); it++)
    {
        ...
        char baoti[510];
        int index = ser.read(baoti).size();
        Data_json json(new char[index + 4], index + 4);
        sprintf(json.data, "%4d%.*s", index, index, baoti);
        char_list.push_back(json);
        do_write();
        // }
    }
}
